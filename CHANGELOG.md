# CHANGELOG
* `0.10.2` (aviationexam fork):
  * Hotfix for broken `0.10.1` release workflow: install `git` in the `Dockerfile` builder stage ([#30](https://github.com/aviationexam/deckschrubber/pull/30)). `go install` falls back to direct VCS (git ls-remote + git clone) when `proxy.golang.org` returns a cache miss for the requested version, and the Go proxy indexes freshly-pushed tags lazily (seconds to minutes), so every Release workflow run that fires immediately after `git push origin <tag>` races the proxy. Without `git` on PATH the fallback aborted with `exec: "git": executable file not found in $PATH` and the GHCR image never published — same failure mode that hit `v0.9.1` (rescued by `v0.9.2`) and `v0.10.1`. No code changes versus `0.10.1` — the installed binary is byte-identical; this release only fixes the release-workflow mechanics so the image actually publishes.
* `0.10.1` (aviationexam fork):
  * Split logrus output streams: `Info`/`Debug`/`Trace` now go to stdout, `Warn`/`Error`/`Fatal`/`Panic` to stderr ([#28](https://github.com/aviationexam/deckschrubber/pull/28)). Logrus ships with a single-sink default (everything to stderr), which conflated normal operational output (`Deleting image`, `Tag not outdated`) with genuine diagnostics (`Could not delete image!`) and made it painful to wire the tool into cron wrappers or log shippers that treat the two fds differently. Implemented via `logrus/hooks/writer` (already available at the `v1.9.4` pinned in `go.mod`): default writer redirected to `io.Discard` and two `writer.Hook` instances handle disjoint level sets. Wire format (logfmt, field order, timestamp) is byte-for-byte identical to 0.10.0 — only the destination fd changes. `log.Fatalf` still exits non-zero because logrus fires hooks before `os.Exit`. Pinned by `integration/log_streams_test.go`.
* `0.10.0` (aviationexam fork):
  ...
  * Replace the bespoke `util/BasicAuthTransport` with `authn.Basic` passed to `remote.WithAuth`. `go-containerregistry`'s internal `basicTransport` already host-gates the `Authorization` header against the registry host (see `pkg/v1/remote/transport/basic.go`), so the library gives us the same safety property the custom transport was written for; keeping it added 46 lines of code for no behavioural gain. The `-user`/`-password` + `-insecure` + `http.ProxyFromEnvironment` path is now a plain `http.Transport` in `main.go`. Pinned by three new integration tests (correct creds, wrong password → 401, no creds → 401) that previously didn't exist because the harness ran an open registry.
  * Reject path-prefixed `-registry` URLs (e.g. `https://example.com/docker` behind a reverse proxy) at start-up with a clear error instead of silently dropping the path on the wire. `go-containerregistry`'s `name.NewRegistry` hardcodes `/v2/` at the host root (it validates its argument as an RFC 3986 authority, see `pkg/name/registry.go checkRegistry()`), and there is no knob to change that. Supporting path-prefixed registries would require upstream changes to `go-containerregistry`. Pinned by `integration/registry_url_test.go`.
